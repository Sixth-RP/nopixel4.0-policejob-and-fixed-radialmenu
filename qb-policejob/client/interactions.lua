...
RegisterNetEvent('police:client:HandcuffMenu', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
...
RegisterNetEvent('police:client:CuffOrUncuff', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QBCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                -- Check if target is cuffed using callback
                QBCore.Functions.TriggerCallback('police:server:IsPlayerCuffed', function(isCuffed)
                    if isCuffed then
                        -- Target is cuffed, uncuff them
                        TriggerServerEvent("police:server:UnCuffPlayer", playerId, false)
                        UnCuffedAnimation(playerId)
                    else
                        -- Target is not cuffed, cuff them
                        TriggerServerEvent("police:server:CuffPlayer", playerId, false)
                        HandCuffAnimation()
                    end
                end, playerId)
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                QBCore.Functions.GetPlayerData(function(PlayerData)
                    if PlayerData.job.name == "police" then
                        -- Police uncuff (requires items)
                        TriggerServerEvent("police:server:UnCuffPlayer", playerId, true)
                    else
                        -- Non-police uncuff (no items required)
                        TriggerServerEvent("police:server:NonPoliceUnCuff", playerId)
                    end
                end)
                TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
            else
...
RegisterNetEvent('police:client:GetUnCuffed', function(playerId, isSoftcuff)
    local ped = PlayerPedId()
    if not IsPedRagdoll(PlayerPedId()) then
        isHandcuffed = false
        isEscorted = false
