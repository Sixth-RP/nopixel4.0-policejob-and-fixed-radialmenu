...
QBCore.Commands.Add("uncuff", "Uncuff the nearest player", {}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("noncuff", "Uncuff the nearest player (non-police)", {}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        TriggerClientEvent("police:client:UnCuffPlayer", src)
...
-- Items
-- Handcuff and handcuff key items removed - using radial menu instead

-- Handcuff useable item (cuff or uncuff)
QBCore.Functions.CreateUseableItem(Config.HandCuffItem, function(source, item)
    local src = source
    TriggerClientEvent("police:client:HandcuffMenu", src)
end)
...
QBCore.Functions.CreateCallback('police:server:IsPlayerCuffed', function(source, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["ishandcuffed"] or false)
end)
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if CuffedPlayer then
        if Player.Functions.GetItemByName(Config.HandCuffItem) then
            TriggerClientEvent("police:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
        else
            TriggerClientEvent('QBCore:Notify', src, "You must have handcuffs to cuff players!", "error")
        end
    end
end)
...
-- Handcuff System
RegisterNetEvent('police:server:SetHandcuffStatus', function(status)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        Player.Functions.SetMetaData("ishandcuffed", status)
        TriggerClientEvent('QBCore:Notify', src, status and "You are now cuffed!" or "You are now uncuffed!", status and 'primary' or 'success')
    end
end)
...
RegisterNetEvent('police:server:CuffPlayer', function(targetId, isSoftcuff)
    ...
    -- Check if player has a law enforcement job
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso" or Player.PlayerData.job.name == "sasp" or Player.PlayerData.job.name == "saspr" or Player.PlayerData.job.name == "sdso" or Player.PlayerData.job.name == "pbso" then
        -- Trigger the cuffed event on the target player
        TriggerClientEvent('police:client:GetCuffed', TargetPlayer.PlayerData.source, src, isSoftcuff)
        TriggerClientEvent('QBCore:Notify', src, "Player handcuffed!", "success")
    else
        TriggerClientEvent('QBCore:Notify', src, "You must be in a law enforcement job to cuff players!", "error")
    end
end)

RegisterNetEvent('police:server:UnCuffPlayer', function(targetId, isSoftcuff)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local TargetPlayer = QBCore.Functions.GetPlayer(targetId)
    ...
    if Player.Functions.GetItemByName(Config.HandCuffItem) or Player.Functions.GetItemByName("handcuffkey") then
        -- Trigger the uncuffed event on the target player
        TriggerClientEvent('police:client:GetUnCuffed', TargetPlayer.PlayerData.source)
        TriggerClientEvent('QBCore:Notify', src, "Player uncuffed!", "success")
    else
        TriggerClientEvent('QBCore:Notify', src, "You must have handcuff keys or handcuffs to uncuff players!", "error")
    end
end)

RegisterNetEvent('police:server:NonPoliceUnCuff', function(targetId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local TargetPlayer = QBCore.Functions.GetPlayer(targetId)
    ...
    -- Non-police uncuff - no item requirement
    TriggerClientEvent('police:client:GetUnCuffed', TargetPlayer.PlayerData.source)
    TriggerClientEvent('QBCore:Notify', src, "Player uncuffed!", "success")
end)
